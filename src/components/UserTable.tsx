...
export default function UserTable({
  ...
  const [dueFilter, setDueFilter] = useState<string | null>("all");
  const [followUpFilter, setFollowUpFilter] = useState<string | null>("all");
  ...
  const filteredUsers = Array.isArray(users)
    ? users
        .filter((user) => {
          ...
          const matchesTag =
            tagFilter === "all" || (user.tags && user.tags.includes(tagFilter));

          let matchesDue = true;
          const today = new Date();
          today.setHours(0, 0, 0, 0);
          const nextDate = user.nextFollowUpDate
            ? new Date(user.nextFollowUpDate)
            : null;

          if (dueFilter === "today") {
            matchesDue = nextDate
              ? nextDate.setHours(0, 0, 0, 0) === today.getTime()
              : false;
          } else if (dueFilter === "overdue") {
            matchesDue = nextDate
              ? nextDate < new Date() &&
                !["converted", "not-interested"].includes(user.status)
              : false;
          }

          const matchesFollowUp =
            followUpFilter === "all" ||
            (user.followUpCount || 0) < (settings?.maxFollowUps || 4);
  ...
  const overdueCount = users.filter((u) => {
    const d = u.nextFollowUpDate ? new Date(u.nextFollowUpDate) : null;
    return (
      d && d < new Date() && !["converted", "not-interested"].includes(u.status)
    );
  }).length;
                  ...
                  <DialogTrigger
                    render={
                      <Button
                        variant="default"
                        size="sm"
                        className="flex-1 sm:flex-none gap-2 h-9 bg-indigo-600 hover:bg-indigo-700 transition-all"
                      />
                    }
                  >
                    <Send size={14} />
                    ...
                    <div className="space-y-4 py-4">
                      <div className="p-3 bg-amber-50 rounded-lg border border-amber-100 flex gap-2 items-start mb-4">
                        ...
                        <p className="text-[11px] text-amber-800 leading-tight">
                          ...
                          <strong>{selectedUsers.length} users</strong>. You
                          should still manually send the messages on Instagram
                      ...
                      <div className="grid gap-2">
                        ...
                        <Select
                          ...
                          onValueChange={setBulkTemplateId}
                        >
                          <SelectTrigger>
                      ...
                      <div className="max-h-[200px] overflow-y-auto border rounded-md p-2 space-y-2">
                        {selectedUsers.map((id) => {
                          ...
                          return (
                            <div
                              key={id}
                              className="flex items-center justify-between text-xs p-2 bg-zinc-50 rounded border"
                            >
              ...
              <Select value={followUpFilter} onValueChange={setFollowUpFilter}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="pending">
                    Pending (&lt;{settings?.maxFollowUps || 4})
                  </SelectItem>
                </SelectContent>
              </Select>
              ...
              {filteredUsers.length === 0 ? (
                ...
                filteredUsers.map((user) => (
                  ...
                  >
                      ...
                      <div className="flex flex-col">
                        ...
                        <div className="flex gap-1 mt-0.5">
                          {user.tags?.map((tag: string) => (
                            <span
                              key={tag}
                              className="text-[9px] px-1 bg-zinc-100 text-zinc-500 rounded uppercase font-bold tracking-wider"
                            >
                              {tag}
                            </span>
                          ))}
                    ...
                    <TableCell>
                      <Badge
                        className={`${getStatusColor(user.status)} border-none shadow-none font-medium`}
                      >
                        {user.status.replace("-", " ")}
                      </Badge>
                    </TableCell>
                      ...
                      <div className="flex items-center gap-2">
                        <div className="w-12 h-1.5 bg-zinc-100 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${Math.min(((user.followUpCount || 0) / (settings?.maxFollowUps || 4)) * 100, 100)}%`,
                            }}
                          />
                        </div>
                        <span className="text-xs font-medium text-zinc-500">
                          {user.followUpCount || 0}/
                          {settings?.maxFollowUps || 4}
                        </span>
                      </div>
                    </TableCell>
                    <TableCell>
                      <span
                        className={`text-xs font-medium ${
                          ...
                          new Date(user.nextFollowUpDate) < new Date() &&
                          !["converted", "not-interested"].includes(user.status)
                            ? "text-red-600"
                            : "text-zinc-500"
                        }`}
                      >
                        {user.nextFollowUpDate
                          ? format(new Date(user.nextFollowUpDate), "MMM d")
                          : "Not set"}
                      </span>
                    </TableCell>
                          ...
                          <DialogTrigger
                            render={
                              <Button
                                variant="ghost"
                                size="icon"
                                className="h-8 w-8 hover:bg-indigo-50 hover:text-indigo-600 transition-all cursor-pointer"
                              />
                            }
                          >
                            <Send
                              ...
                              <div className="grid gap-2">
                                ...
                                <Select
                                  onValueChange={(val: string | null) => {
                                    if (val) handleTemplateSelect(val, user);
                                  }}
                                >
                                  <SelectTrigger>
                                    <SelectValue placeholder="Choose a message template" />
                              ...
                              {previewMessage && (
                                <div className="space-y-3 animate-in fade-in slide-in-from-top-2 duration-300">
                                  ...
                                  <div className="grid grid-cols-1 gap-3">
                                    ...
                                    >
                                      <MessageCircle size={20} /> Copy Message &
                                      Open Profile
                                    </Button>
                                  ...
                                  <div className="p-3 bg-amber-50 rounded-lg border border-amber-100 flex gap-2 items-start">
                                    ...
                                    <div className="text-[11px] text-amber-800 leading-tight">
                                      ...
                                      <p>
                                        Instagram blocks automated bots. Sending
                                        manually ensures your account stays safe
                                        from bans while the CRM handles the
                                        tracking.
                                      </p>
                                    </div>
                                  </div>
